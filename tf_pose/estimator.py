...
#             text_file.close()
            ...
            # addar drawing text
#             fontScale = 1
#             cv2.putText(npimg,'Handsup', (int(flat[28]),int(flat[29])),cv2.FONT_ITALIC, fontScale,(0,0,255), 2, cv2.LINE_AA)    
            
            
            # addar 예외처리 joint value가 0.0일 경우 제외     handsup
            if ((flat[5]!=0.0) & (flat[7]!=0.0) & (flat[9]!=0.0) & (flat[11]!=0.0) & (flat[13]!=0.0) & (flat[15]!=0.0)) :
                # addar 3,4,6, 7번이 1번보다 높이 있으면 o자
                if ((flat[5]  > flat[7]) & (flat[5] > flat[9]) & (flat[11] > flat[13]) & (flat[11] > flat[15]) ):
                    fontScale = 1
                    cv2.putText(npimg,'Handsup', (int(flat[28]),int(flat[29]-10)),cv2.FONT_ITALIC, fontScale,(0,0,255), 2, cv2.LINE_AA)   
                else:
                    fontScale =1
                    cv2.putText(npimg,'General', (int(flat[28]),int(flat[29]-10)),cv2.FONT_ITALIC, fontScale,(255,0,255), 2, cv2.LINE_AA)   
                # addar 1번의 y좌표 띄를 만들고, 8 9 10 11 12 13 쪽이보다 밑에있으면
            else:
                fontScale =1
                cv2.putText(npimg,'Joint_Lost', (int(flat[28]),int(flat[29]-10)),cv2.FONT_ITALIC, fontScale,(255,255,0), 2, cv2.LINE_AA) 

#             ## addar 쓰러짐
#             if :
#                 fontScale =1
#                 cv2.putText(npimg,'Fallwdown', (int(flat[28]),int(flat[29]-10)),cv2.FONT_ITALIC, fontScale,(255,0,0), 2, cv2.LINE_AA) 
